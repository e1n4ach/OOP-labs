...
// Helper functions to convert between digit characters and integer values
...
bool Eleven::isInvalidDigit(unsigned char c) {
    return !((c >= '0' && c <= '9') || c == 'A'); // Only '0'-'9' and 'A' are valid
}
...
Eleven::Eleven(const std::string &str) {
    ...
    _sz = str.size();
    _arr = new unsigned char[_sz];
    std::copy(str.rbegin(), str.rend(), _arr); // Reverse the string into _arr
}
